      ...
      <div className="relative aspect-[2/3] rounded-[2rem] overflow-hidden border border-white/5 group-hover:border-primary/50 transition-all duration-700 shadow-2xl group-hover:shadow-primary/20">
        ...
        <div className="absolute inset-0 z-30 bg-black/60 opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col items-center justify-center gap-4 backdrop-blur-[2px]">
          ...
          <Link href={`/movies/${movie.id}`}>
            <Button variant="outline" className="rounded-full border-white/20 bg-white/5 text-white hover:bg-white/10 text-[10px] font-bold uppercase tracking-wider px-8 py-2.5 h-auto transition-all">
              Chi tiết
            </Button>
          </Link>
        </div>
...
export default function HomePage() {
  ...
  const { data: trendingData } = useGetMoviesQuery({ limit: 10, sort: 'rating' });
  const { data: nowShowingData } = useGetMoviesQuery({ limit: 12, status: 'now-showing' });
  const { data: comingSoonData } = useGetMoviesQuery({ limit: 12, status: 'coming-soon' });
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white selection:bg-primary selection:text-white pb-20">
      ...
      <section className="container mx-auto px-6 md:px-10 py-24 space-y-24">
        <div className="space-y-12">
          ...
          {nowShowingData?.movies && <MovieSlider movies={nowShowingData.movies} onSelect={handleSelectMovie} />}
        </div>
        ...
        <div className="space-y-12">
          ...
          {comingSoonData?.movies && <MovieSlider movies={comingSoonData.movies} onSelect={handleSelectMovie} />}
        </div>
        ...
        <div className="space-y-12">
          ...
          {trendingData?.movies && <MovieSlider movies={trendingData.movies} onSelect={handleSelectMovie} />}
        </div>
      </section>
      ...
      <section ref={sectionCRef} className="container mx-auto px-6 md:px-10 py-24 scroll-mt-20">
        <div className="space-y-8">
           ...
           <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div className={`lg:col-span-7 grid grid-rows-2 gap-6 transition-all duration-700 ${!selectedMovie ? 'opacity-40 grayscale pointer-events-none' : 'opacity-100 grayscale-0'}`}>
                 <div className="rounded-[3rem] border border-white/5 bg-zinc-950/40 p-8 md:p-10 flex flex-col justify-between group hover:border-cinema-gold/30 transition-all shadow-xl">
                    ...
                    <div className="flex flex-wrap gap-3 mt-6">
                       {['RoPhim Hùng Vương', 'RoPhim Thủ Đức'].map(r => (
                         <Badge key={r} onClick={() => setSelectedCinema(r)} variant="outline" className={`px-4 py-2 rounded-xl text-[10px] font-bold transition-all cursor-pointer ${selectedCinema === r ? 'bg-cinema-gold text-black border-cinema-gold' : 'bg-white/5 border-white/5 text-white/40 hover:text-white hover:border-cinema-gold'}`}>{r}</Badge>
                       ))}
                    </div>
                 </div>
                 <div className="rounded-[3rem] border border-white/5 bg-zinc-950/40 p-8 md:p-10 flex flex-col md:flex-row gap-8 items-center shadow-xl">
                    <div className="flex-1 space-y-6 w-full">
                       <div className="space-y-1"><span className="text-[10px] font-bold text-blue-400 uppercase tracking-widest">Thời gian</span><h4 className="text-2xl font-bold text-white uppercase tracking-tight">Chọn Ngày Xem</h4></div>
                       <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
                          {availableDates?.map((date, i) => {
                            const d = new Date(date);
                            return (
                              <div key={date} onClick={() => setSelectedDate(i)} className={`flex-shrink-0 w-14 h-20 rounded-2xl border flex flex-col items-center justify-center gap-1 cursor-pointer transition-all ${selectedDate === i ? 'bg-blue-500 border-blue-400 text-white' : 'bg-white/5 border-white/5 text-white/30 hover:border-white/20'}`}>
                    ...
                    <div className="w-full md:w-auto"><Button onClick={() => { setSelectedTime(null); setModalStep('times'); setShowTimeModal(true); }} className="w-full md:w-40 h-40 rounded-[2.5rem] bg-gradient-to-b from-[#fceabb] to-[#f8d49d] text-[#1a1a1a] flex flex-col items-center justify-center gap-3 shadow-[0_15px_40px_rgba(252,234,187,0.2)] hover:scale-105 active:scale-95 transition-all disabled:opacity-50" disabled={!selectedMovie}><Ticket className="w-10 h-10 transform -rotate-12" /><span className="text-xs font-bold uppercase tracking-widest">Đặt Vé</span></Button></div>
                 </div>
              </div>
           </div>
        </div>
